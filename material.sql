...
CREATE TABLE `material_rfq_quote`  (
  ...
  `tax` bigint NOT NULL DEFAULT 0 COMMENT '税率,单位是百分之一',
  `tax_amount` bigint NOT NULL COMMENT '含税总价,数量*单价*(100+税率),单位是百分之一',
...
CREATE TABLE `material_delivery`  (
  ...
  `quantity` bigint NOT NULL COMMENT '项目部分拨总量',
...
CREATE TABLE `material_delivery_item`  (
  ...
  `quantity` bigint NOT NULL COMMENT '单次送货数量',
